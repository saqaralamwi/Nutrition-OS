...
import NutritionalPlan from '../../data/models/NutritionalPlan';
import VitalsRecord from '../../data/models/VitalsRecord';

import { InsulinCarbRatioEngine } from '../../domain/calculators/InsulinCarbRatioEngine';
import { InsulinSensitivityEngine } from '../../domain/calculators/InsulinSensitivityEngine';
import { Type2DiabetesEngine } from '../../domain/calculators/Type2DiabetesEngine';
import { GestationalDiabetesEngine } from '../../domain/calculators/GestationalDiabetesEngine';
import { GestationalMetabolicTracker } from '../../domain/monitors/GestationalMetabolicTracker';
...
export default function DiabetesForm({ patientId, mode, onStepComplete }: DiabetesFormProps) {
  ...
  const [plannedCarbs, setPlannedCarbs] = useState('60');
    ...
    const stream = combineLatest([patient$, latestPlan$, vitals$]).subscribe({
      next: ([p, plan, vitals]) => {
        ...
        if (plan) {
          ...
          if (profile) {
            setDiabetesType(profile.diabetesType as 'type_1' | 'type_2' | 'gestational');
  ...
  const gestationalOutputs = useMemo(() => {
    ...
    const bmrVal = calculateMifflinBMR(preWtVal, height, age, false);
    ...
    return { dynamics: gdmRes, metabolism: trackerRes };
    ...
    try {
      ...
      await db.write(async () => {
        ...
        const specializedPayload: Record<string, unknown> = {
          ...
          clinicianSignature: signature.trim() || 'wizard_auto_save',
          justificationText: justification.trim() || 'حفظ تلقائي من المعالج التفاعلي',
        ...
        let targetCarbs = targetCals * 0.5 / 4;
        if (diabetesType === 'type_2' && type2Outputs) {
        ...
        if (latestPlan) {
          await latestPlan.update((record) => {
            ...
            record.status = mode === 'wizard' ? 'draft' : record.status;
          });
        } else {
          await plansCollection.create((record) => {
            ...
            record.specializedMetadata = metadataStr;
            record.status = mode === 'wizard' ? 'draft' : 'active';
          });
        }
        ...
        if (mode === 'wizard') {
          ...
          await patientRecord.update((r: any) => {
            const sections: string[] = r.incompleteSections
              ...
              : [];
            const filtered = sections.filter((s: string) => s.trim() !== 'diabetes');
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
      <View style={styles.card}>
        ...
        <View style={styles.tabRow}>
          {(['type_1', 'type_2', 'gestational'] as const).map((type) => (
            <TouchableOpacity
              key={type}
              style={[styles.tabButton, diabetesType === type && styles.tabButtonActive]}
              onPress={() => setDiabetesType(type)}
            >
              <ArabicText
                bold={diabetesType === type}
                style={[styles.tabText, diabetesType === type && styles.tabTextActive]}
              >
        ...
        <View style={styles.grid}>
          <View style={styles.gridItem}>
            <TextInputField label="سكر الصائم (ملغم/دل)" value={fastingBG} onChangeText={setFastingBG} keyboardType="numeric" />
          </View>
          <View style={styles.gridItem}>
            <TextInputField label="سكر فاطر / عشوائي" value={postPrandialBG} onChangeText={setPostPrandialBG} keyboardType="numeric" />
          </View>
          <View style={styles.gridItem}>
            <TextInputField label="معدل التراكمي HbA1c (%)" value={hba1c} onChangeText={setHba1c} keyboardType="numeric" />
          </View>
        </View>
        ...
        {diabetesType === 'type_1' && (
          <View style={styles.subGrid}>
            <View style={styles.gridItem}>
              <TextInputField label="إجمالي الجرعة اليومية (TDD)" value={tdd} onChangeText={setTdd} keyboardType="numeric" />
            </View>
            <View style={styles.gridItem}>
              <TextInputField label="مستوى السكر المستهدف" value={targetBG} onChangeText={setTargetBG} keyboardType="numeric" />
            </View>
          </View>
        )}
        ...
        {diabetesType === 'type_2' && (
          <View style={styles.switchRow}>
            <ArabicText style={styles.switchLabel}>مقاومة الإنسولين الموثقة سريرياً (Insulin Resistance)</ArabicText>
            <Switch
        ...
        {diabetesType === 'gestational' && (
          <View>
            <View style={styles.subGrid}>
              <View style={styles.gridItem}>
                <TextInputField label="أسابيع الحمل" value={gestationalWeeks} onChangeText={setGestationalWeeks} keyboardType="numeric" />
              </View>
              <View style={styles.gridItem}>
                <TextInputField label="الوزن قبل الحمل (كغم)" value={prePregnancyWeight} onChangeText={setPrePregnancyWeight} keyboardType="numeric" />
              </View>
              <View style={styles.gridItem}>
                <TextInputField label="مؤشر كتلة الجسم قبل الحمل" value={prePregnancyBMI} onChangeText={setPrePregnancyBMI} keyboardType="numeric" />
              </View>
            </View>
            <View style={styles.switchRow}>
      ...
      {diabetesType === 'type_2' && type2Outputs && (
        <View style={styles.card}>
          ...
          <ArabicText bold style={styles.subSectionTitle}>
            توزيع المغذيات الكبرى المستهدف ({hasInsulinResistance ? '45%' : '50%'} كربوهيدرات):
          </ArabicText>
          ...
          <View style={styles.alertList}>
            {type2Outputs.clinicalRecommendations.map((rec: string, i: number) => (
      ...
      {diabetesType === 'gestational' && gestationalOutputs && (
        <View style={styles.card}>
          ...
          {ketonesPresent && (
            <Animated.View style={[styles.ketoneAlertContainer, { opacity: pulseAnim }]}>
              <ArabicText bold style={styles.ketoneAlertText}>
                تحذير حرج: وجود أجسام كيتونية في البول الصباحي يشير إلى كيتوزية الجوع نتيجة التقييد المفرط للكربوهيدرات. يجب رفع حصص الكربوهيدرات المعقدة فوراً لحماية الجهاز العصبي للجنين.
              </ArabicText>
            </Animated.View>
          )}
          ...
          <View style={styles.doubleMetricContainer}>
            ...
            <View style={styles.metricCard}>
              <Text style={[styles.metricVal, gestationalOutputs.dynamics.actualWeightGainSoFarKg < 0 ? { color: colors.danger } : {}]}>
          ...
          <View style={styles.alertList}>
            {gestationalOutputs.metabolism.clinicalAlerts.map((rec: string, i: number) => (
              <ArabicText key={i} style={[styles.alertListItem, rec.includes('حرج') ? { color: colors.danger } : {}]}>
                • {rec}
              </ArabicText>
            ))}
          </View>
      ...
      <View style={styles.card}>
        <ArabicText bold style={styles.cardTitle}>تعديل وتجاوز المعايير الغذائية</ArabicText>

        <View style={styles.grid}>
          <View style={styles.gridItem}>
            <TextInputField label="تجاوز السعرات اليومية" value={calorieOverride} onChangeText={setCalorieOverride} keyboardType="numeric" placeholder="تلقائي" />
          </View>
          {diabetesType === 'type_1' && (
            <View style={styles.gridItem}>
              <TextInputField label="تجاوز ICR" value={icrOverride} onChangeText={setIcrOverride} keyboardType="numeric" placeholder="تلقائي" />
            </View>
          )}
          <View style={styles.gridItem}>
            <TextInputField label="تجاوز الكربوهيدرات (غ)" value={carbOverride} onChangeText={setCarbOverride} keyboardType="numeric" placeholder="تلقائي" />
          </View>
        </View>

        {mode === 'screen' ? (
          <Button title="حفظ التعديلات والتوقيع سريرياً" onPress={() => setIsSaveModalOpen(true)} variant="primary" />
        ) : (
          <Button title="حفظ وإكمال الخطوة" onPress={persistClinicianAdjustments} variant="primary" loading={isSaving} />
        )}
      </View>
    </ScrollView>
  );
        ...
        <Modal animationType="fade" transparent visible={isSaveModalOpen} onRequestClose={() => setIsSaveModalOpen(false)}>
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <TextInputField label="التبرير السريري للتجاوز (السبب الطبي)" value={justification} onChangeText={setJustification} placeholder="أدخل مبرر التغيير السريري..." multiline />
              ...
              <View style={styles.modalActions}>
                ...
                <Button title="توقيع وحفظ السجل" onPress={persistClinicianAdjustments} variant="primary" loading={isSaving} style={{ flex: 2 }} />
              </View>
            </View>
          </View>
        </Modal>
...
const styles = StyleSheet.create({
  ...
  cardTitle: { fontSize: 15, color: colors.textPrimary, marginBottom: spacing.md },
  label: { fontSize: 14, color: colors.textSecondary, marginBottom: spacing.xs },
  tabRow: {
    flexDirection: 'row', backgroundColor: colors.surfaceSecondary, borderRadius: 8,
  ...
  subGrid: {
    flexDirection: 'row-reverse', flexWrap: 'wrap', marginHorizontal: -spacing.xs,
    marginTop: spacing.sm, borderTopWidth: 1, borderColor: colors.border, paddingTop: spacing.md,
  },
  switchRow: {
  ...
  badgeRow: { flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', marginBottom: spacing.md },
  ...
  metricCardWarning: { borderColor: colors.warning },
  ...
  helperText: { fontSize: 12, color: colors.textSecondary, marginBottom: spacing.md, textAlign: 'right' },
  calcGrid: {
    backgroundColor: colors.surfaceSecondary, borderRadius: 8, padding: spacing.md,
    borderWidth: 1, borderColor: colors.border,
  },
  subSectionTitle: { fontSize: 13, color: colors.textPrimary, marginBottom: spacing.sm },
  stepperContainer: { marginBottom: spacing.md },
  stepperActions: { flexDirection: 'row', justifyContent: 'flex-end', gap: spacing.sm, marginTop: -spacing.xs, marginBottom: spacing.sm },
  stepBtn: { backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, paddingHorizontal: spacing.md, paddingVertical: spacing.xs, borderRadius: 6 },
  stepBtnText: { fontSize: 12, color: colors.textPrimary },
  outputEstimationContainer: { flexDirection: 'row-reverse', gap: spacing.sm },
  estimationItem: { flex: 1, padding: spacing.sm, backgroundColor: colors.surface, borderRadius: 6, alignItems: 'center' },
  estimationItemHighlight: { backgroundColor: 'rgba(27, 107, 74, 0.15)', borderWidth: 1.5, borderColor: colors.primary },
  estVal: { fontSize: 18, fontWeight: 'bold', color: colors.textPrimary },
  estValHighlight: { color: colors.success },
  estLabel: { fontSize: 10, color: colors.textSecondary, marginTop: 2, textAlign: 'center' },
  estLabelHighlight: { fontSize: 10, color: colors.success, marginTop: 2, textAlign: 'center' },
  macroProgressContainer: { flexDirection: 'row', height: 48, borderRadius: 8, overflow: 'hidden', marginBottom: spacing.md },
  macroBar: { justifyContent: 'center', alignItems: 'center', paddingHorizontal: spacing.xs },
  macroBarText: { fontSize: 10, color: colors.primaryContrast },
  macroBarGram: { fontSize: 11, fontWeight: 'bold', color: colors.primaryContrast },
  alertList: { marginTop: spacing.sm, padding: spacing.sm, backgroundColor: colors.surfaceSecondary, borderRadius: 8, borderWidth: 1, borderColor: colors.border },
  alertListItem: { fontSize: 12, color: colors.textSecondary, marginBottom: spacing.xs, lineHeight: lineHeights.relaxed },
  ketoneAlertContainer: { backgroundColor: 'rgba(239, 68, 68, 0.15)', borderColor: colors.danger, borderWidth: 1.5, borderRadius: 8, padding: spacing.md, marginBottom: spacing.md },
  ketoneAlertText: { fontSize: 13, color: colors.danger, lineHeight: 20, textAlign: 'right' },
  pregnancyStatusContainer: { backgroundColor: colors.surfaceSecondary, borderRadius: 8, padding: spacing.md, marginBottom: spacing.md, borderWidth: 1, borderColor: colors.border },
  targetRow: { flexDirection: 'row-reverse', justifyContent: 'space-between', paddingVertical: spacing.xs, borderBottomWidth: 0.5, borderColor: colors.border },
  targetText: { fontSize: 13, color: colors.textSecondary },
  targetNum: { fontSize: 13, fontWeight: 'bold', color: colors.textPrimary },
